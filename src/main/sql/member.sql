...
select id,用户名,真实姓名,身份证号码,电话号码,
       CONCAT('{"基本信息":{"注册时间":"',`注册时间`,'","邮箱账号":"',ifnull(邮箱账号,''),'","用户类型":"',ifnull(类型,''),'","合作机构":"',ifnull(合作机构,''),'","注册地址":"',ifnull(注册地址,''),'"},',
              '"银行账户":{"银行卡号":"',ifnull(绑定的银行卡号,''),'","支行名称":"',ifnull(支行名称,''),'","开户行地址":"',ifnull(开户行地址,''),'","银行名称":"',ifnull(银行名称,''),'","第三方账号":"',ifnull(第三方账号,''),'"},',
              '"资金":{"总充值":"',ifnull(总充值,''),'","总提现":"',ifnull(总提现,''),'","借款总金额":"',ifnull(借款总金额,''),'","借款次数":"',ifnull(借款次数,''),'","应还本息":"',ifnull(应还本息,''),'","已还利息":"',ifnull(已还利息,''),'","总投资金额":"',ifnull(总投资金额,''),'","已收本金":"',ifnull(已收本金,''),'","已收利息":"',ifnull(已收利息,''),'"},',

              '"旧资料":{"uid":"',旧uid,'","汇付天下账户":"',ifnull(`(旧)汇付天下账户`,''),'","汇付天下id":"',ifnull(`(旧)汇付天下id`,''),'","银行":"',ifnull(`(旧)银行`,''),
              '","支行":"',ifnull(`(旧)支行`,''), '","银行卡号":"',ifnull(`(旧)银行卡号`,''),'","借款总金额":"',ifnull(`(旧)借款总金额`,''),'","借款次数":"',ifnull(`(旧)借款次数`,''),
